...
{
	enum fryz_mode mode;
	struct fryz_audio audio;
	struct graph_settings graph;
	Font font;
};
...
#define FRYZ_RELOAD(name, ...)                                          \
	*(void**)&name = dlsym(libfryz, #name);			\
	...
	{								\
		TraceLog(LOG_ERROR,					\
			 "Failed to get addres of " #name ": %s", dlerror()); \
		return 1;						\
	}
...
#define FRYZ_FUNCTION(name, ret, ...) \
	FRYZ_DECLARE_FUNCTION(name, ret, __VA_ARGS__)
